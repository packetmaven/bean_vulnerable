...
// JNI -> native file I/O using tainted path (intentional leak signal for demo).
...
static jstring nativeReflectInvokeImpl(
    JNIEnv *env,
    jobject obj,
    jstring className,
    jstring methodName,
...
// Attacker-influenced pointer target selection for dynamic table entry.
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    jclass targetClass = (*env)->FindClass(env, "com/example/complexjni/VUL_COMPLEX_MultiVulnerability_JNI");
    if (targetClass == NULL) {
        return JNI_ERR;
    }
