...
def main():
    ...
    for i, raw_input in enumerate(args.input, 1):
        ...
        if args.html_report and (args.export_dfg or args.export_cfg or args.export_pdg):
            report_dir = Path(args.html_report).expanduser()
            LOG.info(f"🎨 Generating graphs for {p.name}...")
            ...
            try:
                ...
                for graph_file in ['pdg_slice', 'cfg_slice', 'dfg_slice1']:
                    dot_file = report_dir / f"{graph_file}.dot"
                    if dot_file.exists():
                        # PNG
                        png_file = report_dir / f"{graph_file}.png"
                        try:
                        ...
                        # SVG
                        svg_file = report_dir / f"{graph_file}.svg"
                        try:
