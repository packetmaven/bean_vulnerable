...
class CFExplainer:
    ...
    def _generate_minimal_change_summary(self, modifications: List[Dict]) -> str:
        ...
        if not modifications:
            return 'CF-Explainer is analyzing the vulnerability pattern to generate specific code modifications. This requires the trained GNN model to identify minimal changes that would make the code safe.'
...
class CFExplainerIntegration:
    ...
    def explain_vulnerability(self, file_path: str, analysis_result: Dict) -> Dict:
        ...
        # Get vulnerability score from confidence or vulnerability_score key
        vulnerability_score = analysis_result.get('confidence', analysis_result.get('vulnerability_score', 0.0))
